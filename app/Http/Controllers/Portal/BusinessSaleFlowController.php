...
class BusinessSaleFlowController extends Controller
{
    ...
    public function add(){
        return view('portal.BusinessSale.add');
    }
    public function edit($id){
        $flow=BusinessSaleFlow::findorfail($id);
        return view('portal.BusinessSale.edit',compact('flow'));
    }
    ...
    public function list(Request $request)
    {
        ...
        // Total records
        $totalRecords = BusinessSaleFlow::select('count(*) as allcount')->count();
        $totalRecordswithFilter = BusinessSaleFlow::select('count(*) as allcount')->where('step_name', 'like', '%' . $searchValue . '%')->count();
        $records = BusinessSaleFlow::orderBy($columnName, $columnSortOrder)
        ...
        foreach ($records as $record) {
            ...
            $delete_route = route('portal.business-sale-flow.delete', $record->id); 
            $edit_route = route('portal.business-sale-flow.edit', $record->id); 
            $data_arr[] = array(
                "id" => $record->id,
    ...
    public function store(Request $request)
{
    ...
    if ($flow) {
        return response()->json(['message' => 'Step added successfully', 'redirectURL' => route('portal.business-sale-flow.index')], 200);
    } else {
...
public function delete($id){
    $flow=BusinessSaleFlow::findorfail($id);
    $flow->delete();
    return redirect()->route('portal.business-sale-flow.index')->with('success','Step Deleted');
}
...
public function update(Request $request, $id = null)
{
    ...
    $flow = $id ? BusinessSaleFlow::findOrFail($id) : new BusinessSaleFlow;
    ...
    if ($flow) {
        return response()->json(['message' => 'Step updated successfully', 'redirectURL' => route('portal.business-sale-flow.index')], 200);
    } else {
